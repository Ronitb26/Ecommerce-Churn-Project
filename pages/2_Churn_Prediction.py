...
complain = 'Yes' if st.checkbox("Ever Registered Complain Before?") else 'No'

curr_input = data = [tenure,city_tier,distance,payment_mode,devices,order_cat,satisfaction,marital_status,addresses,
                 complain,hike,count,last_order,cashback]
if st.session_state.saved_inputs and st.session_state.saved_inputs != curr_input:
    st.session_state.button_clicked = False
    st.session_state.saved_inputs = None
...
with col1:
    if st.button("Predict"):
        ...
        st.session_state.saved_inputs = curr_input.copy()
        # form dataframe
        ...
        st.session_state.pred = pipe
        if st.session_state.button_clicked:
            st.text("Churn Probability : {} %".format(f"{st.session_state.prob:.2f}"))
...
with col2:
    if st.session_state.button_clicked:
        ...
        if ft in st.session_state.test_df.select_dtypes('number').columns:
            val = st.slider("Select Value",min(X[ft]),max(X[ft]))
        else:
            val = st.selectbox("Select Value",X[ft].unique().tolist())
        new_df = st.session_state.test_df.copy()
        new_df[ft]=val

        new_prob = (pipeline.predict_proba(new_df)[0][1]* 100)
        change_prob = new_prob-st.session_state.prob
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Inital Churn Probability :",value=" {} %".format(round(float(st.session_state.prob),2)),border=True)
        with col2:
